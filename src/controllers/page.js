...
const FilmsConter = {
  SHOWING_FILM_CARD_COUNT_ON_START: 5,
  SHOWING_FILM_CARD_COUNT_BY_BUTTON: 5,
  SHOWING_FILM_CARD_COUNT_BY_EXTRA: 2,
};
...
export default class PageController {
  constructor(filmsComponent, sortComponent, moviesModel, api) {
    ...
    this._showingFilmCardCountByButton = FilmsConter.SHOWING_FILM_CARD_COUNT_BY_BUTTON;
  ...
  renderTopRatedList() {
    if (this._moviesModel.hasRatings()) {
      ...
      this._renderFilms(topRatedContainerElements, sortedFilms.slice(0, FilmsConter.SHOWING_FILM_CARD_COUNT_BY_EXTRA));
    }
  }
  ...
  renderMostCommentedList() {
    if (this._moviesModel.hasComments()) {
      ...
      this._renderFilms(mostCommentedContainerElements, sortedFilms.slice(0, FilmsConter.SHOWING_FILM_CARD_COUNT_BY_EXTRA));
    }
  }
  ...
  _onSortTypeChange(sortType) {
    ...
    this._showingFilmCardCountByButton = FilmsConter.SHOWING_FILM_CARD_COUNT_ON_START;
  ...
  _onFilterChange() {
    ...
    this._removeFilms();
    this._renderFilms(filmsListContainerElement, films.slice(0, FilmsConter.SHOWING_FILM_CARD_COUNT_ON_START));
    this._renderLoadMoreButton(films);
  }
}
