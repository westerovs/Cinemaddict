...
// блоки экстра фильмы
const extraListElement = siteMainElement.querySelector(`.films .films-list--extra .films-list__container`);
// если блоку выше добавить querySelectorAll - то я получаю два котнейнера.
// Но не получается в него вставить new Array(2)
for (let i = 0; i < extraListElement.length; i++) {
  extraListElement[i].style.height = `300px`;
  extraListElement[i].style.outline = `2px dashed gold`;
}
...
// cчётчик для экстра - фильмов
new Array(2)
  .fill(``)
  .forEach(
    () => render(extraListElement, createFilmCardTaskTemplate())
  );
...
// поп-арт - закоментировал, т.к он всё закрывает
